...
const stackingNavbar = document.getElementById('stackingNavbar');
const navbarItems = document.querySelectorAll('.stacking-navbar__item');
const brandLogo = document.querySelector('.brand a');
...
if (burgerMenu && stackingNavbar) {
  ...
  navbarItems.forEach(item => {
    item.addEventListener('click', () => {
      ...
      stackingNavbar.classList.remove('active');
    });
  });

  // Close when clicking outside
  document.addEventListener('click', (e) => {
    if (!stackingNavbar.contains(e.target) && !burgerMenu.contains(e.target)) {
      burgerMenu.classList.remove('active');
      stackingNavbar.classList.remove('active');
    }
  });
  ...
  stackingNavbar.addEventListener('click', (e) => {
    e.stopPropagation();
  });
}
...
const initHyperspeed = (effectOptions = DEFAULT_EFFECT_OPTIONS) => {
  let hyperspeedRef = null;
  let appRef = null;
